...
int main()
{
  ...
  while (1)
  {
      ...
      cv::findContours(hsv, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, cv::Point(0, 0));
      ...
      if (!contours.empty())
            {
                ...
                if (hull[0].size() > 2)
                {
                    ...
                    for (size_t i = 0; i < validPoints.size(); i++)
                    {
                        ...
                        cv::rectangle(frame, cv::Point(validPoints[i].x-18, validPoints[i].y-36),cv::Point(validPoints[i].x+18, validPoints[i].y+36),cv::Scalar(0,0,255),2 );
                    }
                }
            }
